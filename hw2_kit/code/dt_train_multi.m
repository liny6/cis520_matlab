...
%convert Y into Z first

Z_mesh = meshgrid(1:max(Y), 1:numel(Y));
Z = bsxfun(@eq, Z_mesh, Y);

%Z must be 1's and 0's 
assert(isequal(unique(Z), [0; 1]), 'Y must be integers');
...
root = split_node_multi(X, Y, Z, Xrange, round(mean(Y)), 1:size(Xrange, 2), 0, depth_limit);

function [node] = split_node_multi(X, Y, Z, Xrange, default_value, colidx, depth, depth_limit)
...
%    - we have Z that are all the same
%    - we have only a single (or no) examples left
%    - we have no features left to split on
no_more_to_split = length(unique(Y)) < 1;
if depth == depth_limit || no_more_to_split || length(Z) <= 1 || numel(colidx) == 0
    node.terminal = true;
    node.fidx = [];
    node.fval = [];
    if isempty(Y)
...
node.left = split_node_multi(X(leftidx, :), Y(leftidx), Z(leftidx, :), Xrange, node.value, colidx, depth+1, depth_limit);
node.right = split_node_multi(X(rightidx, :), Y(rightidx), Z(rightidx, :), Xrange, node.value, colidx, depth+1, depth_limit);
